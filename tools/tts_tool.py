...
# Deepgram Aura TTS. https://developers.deepgram.com/docs/tts-rest
...
PROVIDER_MAX_TEXT_LENGTH: Dict[str, int] = {
    ...
    "deepgram": 2000,     # https://developers.deepgram.com/docs/tts-rest — 2000 char/request
}
...
async def _generate_edge_tts(text: str, output_path: str, tts_config: Dict[str, Any]) -> str:
    ...
    # Microsoft's free Edge endpoint intermittently returns no audio
    # (edge_tts.NoAudioReceived) — a transient server hiccup, not a config
    # error. Retry a couple times so a flaky call doesn't leave a reply silent.
    ...
    for attempt in range(3):
        try:
            ...
            if os.path.exists(output_path) and os.path.getsize(output_path) > 0:
                return output_path
            last_exc = RuntimeError("Edge TTS produced an empty file")
        except Exception as exc:  # noqa: BLE001 — retry any transient edge error
            last_exc = exc
        if attempt < 2:
            await asyncio.sleep(0.4 * (attempt + 1))
    raise last_exc or RuntimeError("Edge TTS failed")
...
def _generate_deepgram_tts(text: str, output_path: str, tts_config: Dict[str, Any]) -> str:
    """Generate audio using the Deepgram Aura ``/v1/speak`` REST API.

    Deepgram picks the voice from the ``model`` name (e.g. ``aura-2-thalia-en``);
    there is no separate voice_id parameter.
    """
    import requests

    api_key = get_env_value("DEEPGRAM_API_KEY")
    if not api_key:
        raise ValueError("No Deepgram credentials found. Set DEEPGRAM_API_KEY.")

    deepgram_config = tts_config.get("deepgram", {}) or {}
    model = str(deepgram_config.get("model", DEFAULT_DEEPGRAM_TTS_MODEL)).strip() or DEFAULT_DEEPGRAM_TTS_MODEL
    base_url = str(
    ...
    # Match the output container to the file Hermes allocated. Aura returns MP3
    # by default; for a .wav target request linear16/wav at 24kHz.
    if output_path.endswith(".wav"):
        params = {"model": model, "encoding": "linear16", "container": "wav", "sample_rate": "24000"}
    else:
