...
def _deepgram_content_type(file_path: str) -> str:
    """Best-effort audio Content-Type for a Deepgram prerecorded upload."""
...
def _transcribe_deepgram(file_path: str, model_name: str) -> Dict[str, Any]:
    """Transcribe using the Deepgram prerecorded ``/v1/listen`` API."""
    ...
    smart_format = is_truthy_value(deepgram_config.get("smart_format", True))
    punctuate = is_truthy_value(deepgram_config.get("punctuate", True))

    params: Dict[str, str] = {"model": model_name}
    if smart_format:
    ...
    try:
        ...
        # A transient Deepgram/network hiccup shouldn't lose a whole voice turn —
        # retry a couple times on connection/timeout errors and 5xx responses.
        # Voice clips are short, so a 30s per-attempt timeout is ample.
        response = None
        ...
        for attempt in range(3):
            try:
                response = requests.post(
                    ...
                    headers={
                        ...
                        "Content-Type": _deepgram_content_type(file_path),
                    },
                    params=params,
                    data=audio_bytes,
                    timeout=30,
            ...
            if response.status_code >= 500 and attempt < 2:
                logger.debug("Deepgram STT %s, retrying", response.status_code)
                time.sleep(0.4 * (attempt + 1))
                continue
            break

        if response is None:
            raise last_exc or RuntimeError("Deepgram STT: no response")

        if response.status_code != 200:
            detail = ""
            try:
                err_body = response.json()
                detail = str(err_body.get("err_msg") or err_body.get("reason") or err_body)
            except Exception:
                detail = response.text[:300]
            return {
                "success": False,
        ...
        logger.info(
            "Transcribed %s via Deepgram (%s, %d chars)",
            Path(file_path).name,
            model_name,
            len(transcript_text),
        )
