...
_LONG_HANDLERS = frozenset(
    {
        ...
        "complete.slash",
        "llm.oneshot",
        # Fast direct streaming voice reply — a single streaming
        # chat/completions call that blocks its worker for a few seconds while
        # it streams; must run off the reader thread like llm.oneshot.
        "voice.reply",
...
# How many prior turns (user+assistant messages) the voice path carries as
# context. ~4 exchanges is plenty for follow-ups without ballooning latency.
MAX_VOICE_HISTORY = 8

# Tight spoken-reply persona for the fast voice path. Short output + minimal
# deliberation is the real latency lever — keep this small.
VOICE_SYSTEM_PROMPT = (
    "You are J.A.R.V.I.S., a refined British butler intelligence serving one "
    'man, whom you address as "Sir." Every reply is spoken aloud: plain spoken '
    "British English, no markdown, no lists, no symbols. Answer in one or two "
    "short sentences, conclusion first, with no preamble and without restating "
    "the question. Do not deliberate — reach the answer with the least possible "
    "reasoning and say it immediately. A trace of dry wit is welcome; padding "
    "is not."
)

# Female-persona counterpart selected when voice.persona == 'friday'. Same tight
# spoken-reply shape; warmer, still crisp, addresses the user as "Boss."
VOICE_SYSTEM_PROMPT_FRIDAY = (
    "You are F.R.I.D.A.Y., a sharp, warm AI aide serving one person, whom you "
    'address as "Boss." Every reply is spoken aloud: plain spoken English, no '
    "markdown, no lists, no symbols. Answer in one or two short sentences, "
    "conclusion first, with no preamble and without restating the question. Do "
    "not deliberate — reach the answer with the least possible reasoning and say "
    "it immediately. A little warmth and dry wit is welcome; padding is not."
)


def _voice_system_prompt() -> str:
    """Pick the fast-path spoken persona prompt from voice.persona config."""
    try:
        from agent.prompt_builder import active_voice_persona

        if active_voice_persona() == "friday":
            return VOICE_SYSTEM_PROMPT_FRIDAY
    except Exception:
        pass
    return VOICE_SYSTEM_PROMPT


def _voice_direct_client_and_model():
    """Build a SYNC OpenAI client + model for the fast voice path straight from
    config, honoring the configured provider and its auth handling (the
    opencode-zen ``Authorization`` strip lives in ``build_keepalive_http_client``).

    Independent of whether the session agent has finished building — session
    agents are constructed on a background thread after ``session.create``, so
    the first voice turn can't rely on ``session["agent"].client`` existing yet.
    """
    try:
        from hermes_cli.config import load_config
        ...
        api_key = str(mcfg.get("api_key") or "").strip() or "x"
        if not model or not base_url:
            return None, None

        from agent.process_bootstrap import build_keepalive_http_client, OpenAI
        ...
        logger.warning("voice.reply: could not build direct client: %s", e)
        return None, None


@method("voice.reply")
def _(rid, params: dict) -> dict:
    """Fast, direct streaming LLM reply for the voice assistant.

    Bypasses the full agent tool-calling loop: one streaming chat/completions
    call with ``reasoning_effort=low`` and a tight spoken-reply system prompt,
    streamed to the client as the SAME ``message.delta`` / ``message.complete``
    frames ``prompt.submit`` emits — so the renderer speaks it identically, but
    with far lower latency than a full agent turn (no big system prompt / tool
    schemas, and reasoning is capped). Used for conversational voice replies;
    tool/action requests still go through ``prompt.submit``.
    """
    sid = params.get("session_id") or ""
    text = (params.get("text") or "").strip()
    if not text:
        return _err(rid, 4030, "voice.reply requires text")

    # Conversation memory: the renderer sends the recent turns so JARVIS has
    # context (follow-ups like "and what about Germany?"). Sanitized + capped
    # here so a stray payload can't blow up the prompt.
    history_msgs: list[dict] = []
    raw_history = params.get("history")
    if isinstance(raw_history, list):
        for item in raw_history[-MAX_VOICE_HISTORY:]:
            if not isinstance(item, dict):
                continue
    ...
    session = _sessions.get(sid)
    # Keep streaming events on the active websocket (same rebind prompt.submit does).
    if session is not None and (t := current_transport()) is not None:
        session["transport"] = t

    # Always build a fresh direct client from config. We deliberately do NOT
    # reuse ``session["agent"].client``: the agent client merges its own
    # extra_body and drops our per-request ``reasoning_effort=low`` (leaving the
    # model to reason at full effort and blow the token budget → empty reply).
    # The direct client applies reasoning_effort correctly and is self-contained.
    client, model = _voice_direct_client_and_model()
    if client is None or not model:
        fallback = "My apologies, Sir — the model isn't ready just yet."
        _emit("message.complete", sid, {"text": fallback})
        return _ok(rid, {"text": fallback})
    ...
    try:
        stream = client.chat.completions.create(
            ...
            max_tokens=256,
            temperature=0.4,
            # Top-level `reasoning_effort: low` is what makes opencode-zen fast
            # (cuts the reasoning phase ~60%). Never "none" — that empties content.
        ...
        for chunk in stream:
            ...
            if piece:
                pieces.append(piece)
                _emit("message.delta", sid, {"text": piece})
    except Exception as e:
        logger.warning("voice.reply failed: %s", e)
        fallback = "My apologies, Sir — I couldn't complete that just now."
        _emit("message.complete", sid, {"text": fallback})
        return _ok(rid, {"text": fallback})

    reply = "".join(pieces).strip()
    _emit("message.complete", sid, {"text": reply})
    return _ok(rid, {"text": reply})
