...
def _handle_spotify_playback(args: dict, **kw) -> str:
    ...
    try:
        ...
        if action == "play":
            ...
            # Backstop for under-specified "play": a bare play with no track,
            # album, or playlist is only a *resume* of whatever is already loaded.
            # If nothing is resumable, refuse to guess — instruct the agent to ask
            # the user which song (via the clarify tool) and search first. This is
            # what turns "Jarvis, drop the needle" into "Which song, Sir?" instead
            # of a silent no-op that plays nothing (or resumes something stale).
            if not uris and not context_uri:
                try:
                    state = client.get_playback_state(market=args.get("market"))
                except Exception:
                    state = None
                has_resumable = (
                    isinstance(state, dict)
                    and not state.get("empty")
                    and bool(state.get("item"))
                )
                if not has_resumable:
                    return tool_result({
                        ...
                        "message": (
                            "No song was specified and nothing is currently loaded to "
                            "resume. Do NOT start playback on a guess. Ask the user which "
                            "song, artist, album, or playlist they want (use the clarify "
                            "tool), then search for it and play that specific track."
                        ),
                    })
...
SPOTIFY_PLAYBACK_SCHEMA = {
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "context_uri": {"type": "string", "description": "Spotify URI of the album/playlist/artist to play. For action='play', provide this OR 'uris' to start a specific selection. A 'play' with neither only RESUMES the current track — if the user asked to play something new, first search for it and pass its track uri."},
            "uris": {"type": "array", "items": COMMON_STRING, "description": "Track URIs to play. Required to play a specific song; obtain via spotify_search. Do not call action='play' without a target when the user wants a new song — ask which song first."},
            "offset": {"type": "object"},
