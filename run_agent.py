...
class AIAgent:
    ...
    def _build_keepalive_http_client(base_url: str = "", *, verify: Any = True) -> Any:
        try:
            ...
            _base_lower = str(base_url or "").lower()

            # OpenCode Zen's free gateway (opencode.ai/zen) rejects ANY
            # Authorization header with 401 "Invalid API key" and only accepts
            # requests where the header is ABSENT — but the OpenAI SDK always
            # sends one (any api_key value). Strip it for this host so the
            # keyless free models work. Scoped strictly to opencode.ai; every
            # other provider keeps its auth header untouched.
            _extra_client_kwargs: dict = {}
            if "opencode.ai" in _base_lower:
                def _strip_auth_header(request: Any) -> None:
                    request.headers.pop("authorization", None)

                _extra_client_kwargs["event_hooks"] = {"request": [_strip_auth_header]}

            if "api.githubcopilot.com" in _base_lower:
                return _httpx.Client(verify=verify, **_extra_client_kwargs)
