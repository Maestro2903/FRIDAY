/**
 * ElevenLabs UI — Orb  (vendored from https://ui.elevenlabs.io, r/orb.json)
 *
 * Adapted for the Hermes dashboard:
 *   - Removed the `@react-three/drei` `useTexture` dependency and the remote
 *     perlin-noise texture fetch (storage.googleapis.com). Instead we generate
 *     a smooth value-noise texture locally so the Orb is fully self-contained
 *     and works offline / behind a strict CSP.
 *   - `cn`/import paths unchanged from upstream (this file has no local imports).
 *
 * The shader (vertex/fragment) is upstream ElevenLabs, unmodified.
 */
/* eslint-disable react-hooks/immutability, react-hooks/refs, react-hooks/preserve-manual-memoization --
 * Vendored ElevenLabs Orb. react-three-fiber renders imperatively: it mutates
 * ShaderMaterial uniforms inside useFrame and seeds uniforms from refs/initial
 * values during memo setup. These patterns are intentional for r3f and safe
 * here (the mesh is never re-created), so the React-Compiler lint rules that
 * assume pure render are disabled for this third-party file. */
...
/**
 * Local replacement for drei's `useTexture(<remote perlin png>)`.
 *
 * Builds a tiling, smoothly-interpolated value-noise texture on a canvas. The
 * shader samples this for its flow distortion and ring noise; a smooth noise
 * field is all it needs — the exact perlin field only affects fine detail.
 */
function makeNoiseTexture(size = 256): THREE.Texture {
  ...
  // Coarse random grid, tiling in both axes, then bilinear-smoothstep upsample.
  const grid = 32;
  const rand = new Float32Array(grid * grid);
  for (let i = 0; i < rand.length; i++) rand[i] = Math.random();
  const at = (x: number, y: number) =>
    rand[((y % grid) + grid) % grid * grid + (((x % grid) + grid) % grid)];
