...
_SCHEMA_OVERRIDES: Dict[str, Dict[str, Any]] = {
    ...
    "tts.provider": {
        ...
        "options": ["edge", "elevenlabs", "deepgram", "openai", "neutts"],
    },
    "stt.provider": {
        "type": "select",
...
def _synthesize_speech_with_fallback(text: str) -> Dict[str, Any]:
    """Synthesize speech via the configured TTS provider, falling back to
    Deepgram on failure.

    The free Microsoft Edge voices (used for the Irish "FRIDAY" tone) are
    high-quality but intermittently return no audio. Rather than surface that
    as a "voice playback failed" to the user, retry the request through
    Deepgram (fast + reliable) when a key is configured, so a spoken reply is
    ...
    def _ok(result: Any) -> bool:
        return (
            ...
            and bool(result.get("file_path"))
            and os.path.isfile(result["file_path"])
        )
    ...
    try:
        ...
        _log.warning("Primary TTS provider raised: %s", exc)

    if _ok(result):
        return result

    key = (load_env().get("DEEPGRAM_API_KEY") or os.environ.get("DEEPGRAM_API_KEY") or "").strip()
    if key:
        try:
            from tools.tts_tool import _generate_deepgram_tts

            with tempfile.NamedTemporaryFile(
                prefix="hermes-tts-dg-", suffix=".mp3", delete=False
            ) as tmp:
                out_path = tmp.name
            _generate_deepgram_tts(text, out_path, {"deepgram": {"model": "aura-2-draco-en"}})
            if os.path.isfile(out_path) and os.path.getsize(out_path) > 0:
                _log.info("TTS fell back to Deepgram after primary provider failed")
                return {"success": True, "file_path": out_path, "provider": "deepgram-fallback"}
        except Exception as exc:
            _log.warning("Deepgram TTS fallback failed: %s", exc)
...
async def speak_text(payload: TTSSpeakRequest):
    ...
    try:
        loop = asyncio.get_running_loop()
        result = await loop.run_in_executor(None, _synthesize_speech_with_fallback, text)
    except Exception as exc:
