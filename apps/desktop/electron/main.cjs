...
// Whether Jarvis shows its icon in the macOS Dock. When false, the app runs as a
// menu-bar-only accessory (Tray icon only). Persisted so cold launches honour it.
const DESKTOP_DOCK_CONFIG_PATH = path.join(app.getPath('userData'), 'dock.json')
// The user's curated set of apps shown in the hand-control wheel (by name).
const DESKTOP_CONTROL_APPS_PATH = path.join(app.getPath('userData'), 'control-panel-apps.json')
...
let mainWindow = null
// Menu-bar (tray) icon + a route the tray asked us to open once the (possibly
// just-recreated) main-window renderer signals it's ready. See navigateMainWindow.
let tray = null
let _pendingTrayNav = null
...
// Camera (video) capture — enabled for the hand-control "control panel" feature.
// The macOS TCC prompt (systemPreferences.askForMediaAccess('camera')) still
// gates real access, so the user must grant it once regardless.
function isVideoCapturePermission(permission, details) {
...
function installMediaPermissions() {
  ...
  session.defaultSession.setPermissionRequestHandler((_webContents, permission, callback, details) => {
    callback(isAudioCapturePermission(permission, details) || isVideoCapturePermission(permission, details))
  })
  ...
  session.defaultSession.setPermissionCheckHandler((_webContents, permission, _origin, _details) => {
    // Audio + video are both permitted at the Electron layer; the OS TCC prompt
    // remains the real gate for the camera/mic.
...
// The Siri-style notch HUD: a transparent NSPanel that drops from the notch and
// mirrors the voice orb's state (pushed from the main renderer over IPC).
let notchWindow = null
// The hand-control "control panel": a full-screen transparent overlay with a
// webcam hand skeleton + a radial app wheel.
let handtrackWindow = null
...
// ─── Notch HUD (Siri-style drop-down) ──────────────────────────────────────

const NOTCH_WIDTH = 560
const NOTCH_HEIGHT = 400

function notchUrl() {
  if (DEV_SERVER) {
    return `${DEV_SERVER.endsWith('/') ? DEV_SERVER.slice(0, -1) : DEV_SERVER}/?win=notch#/`
  }
  return `${pathToFileURL(resolveRendererIndex()).toString()}?win=notch#/`
}

// Center the panel under the physical notch on the display that owns the menu
// bar, hugging the very top edge (bounds.y, not workArea) so it reads as the
// notch dropping down.
function positionNotchWindow(win) {
  try {
    const display = screen.getPrimaryDisplay()
    const b = display.bounds
    const [w] = win.getSize()
    win.setPosition(Math.round(b.x + (b.width - w) / 2), Math.round(b.y), false)
  } catch (err) {
    rememberLog(`[notch] position failed: ${err?.message || err}`)
  }
}
...
function spawnNotchWindow() {
  const win = new BrowserWindow({
    ...
    hasShadow: false,
    alwaysOnTop: true,
    // NSPanel: a non-activating helper window that floats over full-screen
    // spaces without stealing the app's Dock/cmd-tab anchor (see pet overlay).
    type: IS_MAC ? 'panel' : undefined,
    hiddenInMissionControl: IS_MAC,
    focusable: false,
    show: false,
  ...
  win.once('ready-to-show', () => {
    if (!win.isDestroyed()) {
      ...
      // Start click-through everywhere; the renderer flips it off while the
      // pointer is over the actual HUD content (see hermes:notch:ignore-mouse).
  ...
  win.loadURL(notchUrl())
  return win
}

function openNotch() {
  if (notchWindow && !notchWindow.isDestroyed()) {
    positionNotchWindow(notchWindow)
...
// Summon Jarvis: open the notch HUD and make sure the main window is running the
// voice loop (the /voice view owns the gateway + mic + TTS and pushes its live
// state to the notch). Triggered by the global hotkey and, later, "Hey Jarvis".
function summonJarvis() {
  openNotch()
  navigateMainWindow('/voice')
}

// ─── Hand-control overlay (the "control panel" app wheel) ───────────────────

function handtrackUrl() {
  if (DEV_SERVER) {
    return `${DEV_SERVER.endsWith('/') ? DEV_SERVER.slice(0, -1) : DEV_SERVER}/?win=handtrack#/`
  }
  return `${pathToFileURL(resolveRendererIndex()).toString()}?win=handtrack#/`
}

function spawnHandtrackWindow() {
  ...
  const win = new BrowserWindow({
    ...
    type: IS_MAC ? 'panel' : undefined,
    hiddenInMissionControl: IS_MAC,
    // Interactive (unlike the notch) — the wheel takes pointer/keyboard + the
    // camera drives gestures.
    focusable: true,
    show: false,
    backgroundColor: '#00000000',
    webPreferences: {
  ...
  win.on('closed', () => {
    if (handtrackWindow === win) {
      handtrackWindow = null
    }
  })

  win.loadURL(handtrackUrl())
  return win
}

function openHandtrack() {
  if (handtrackWindow && !handtrackWindow.isDestroyed()) {
    handtrackWindow.show()
    handtrackWindow.focus()
    return handtrackWindow
  }
  handtrackWindow = spawnHandtrackWindow()
  return handtrackWindow
}

function closeHandtrack() {
  if (handtrackWindow && !handtrackWindow.isDestroyed()) {
    handtrackWindow.close()
  }
  handtrackWindow = null
}

function createWindow() {
  // A fresh renderer is loading; it must re-signal readiness before we deliver
  // any queued deep link / tray navigation (otherwise we'd send to a page that
  // hasn't mounted its listeners yet). The renderer calls signalDeepLinkReady()
  // on mount, which flips this back to true and flushes anything pending.
  ...
  mainWindow.on('closed', () => {
    closePetOverlay()
    // The notch stays (NotchNook-style, independent of the main window); only the
    // hand-control overlay is torn down with the window.
    closeHandtrack()
  })
...
// ─── Menu-bar (tray) icon + optional Dock ──────────────────────────────────

// Show the Dock icon by default; a missing/corrupt file means "show".
function readShowDockIcon() {
  try {
    const raw = fs.readFileSync(DESKTOP_DOCK_CONFIG_PATH, 'utf8')
    const parsed = JSON.parse(raw)
    if (parsed && typeof parsed.show === 'boolean') return parsed.show
  } catch {
    /* no file yet / unreadable → default */
  }
  return true
}

function writeShowDockIcon(show) {
  try {
    fs.writeFileSync(DESKTOP_DOCK_CONFIG_PATH, JSON.stringify({ show: Boolean(show) }), 'utf8')
  } catch (err) {
    rememberLog(`[dock] persist failed: ${err?.message || err}`)
  }
}

// macOS-only: toggle Dock presence. Hiding uses the 'accessory' activation
// policy so Jarvis becomes a true menu-bar app (no Dock, no ⌘-Tab entry) yet its
// windows still open from the Tray. 'regular' restores normal Dock behaviour.
function applyDockVisibility(show) {
  ...
  try {
    ...
    rememberLog(`[dock] apply failed: ${err?.message || err}`)
  }
}

// Bring the main window forward (creating it if the user closed it), then send a
// HashRouter route for the renderer to navigate to. If the renderer isn't mounted
// yet (freshly created window), queue the route — it's flushed from the
// deep-link-ready handler once the renderer signals it's listening.
function navigateMainWindow(route) {
  ...
  if (_rendererReadyForDeepLink && mainWindow && !mainWindow.isDestroyed()) {
    try {
      ...
      mainWindow.webContents.send('hermes:tray-navigate', route)
      return
    } catch (err) {
      rememberLog(`[tray] navigate send failed: ${err?.message || err}`)
    }
  }
  _pendingTrayNav = route
}
...
function buildTrayMenu() {
  return Menu.buildFromTemplate([
    ...
    { label: 'New Chat', click: () => navigateMainWindow('/') },
    { label: 'Talk to Jarvis', click: () => summonJarvis() },
    { label: 'Control Panel', click: () => openHandtrack() },
    {
      label: 'Toggle Notch',
      click: () => {
        if (notchWindow && !notchWindow.isDestroyed()) {
    ...
    { label: 'Settings…', click: () => navigateMainWindow('/settings') },
    {
      label: 'Show in Dock',
      type: 'checkbox',
      checked: readShowDockIcon(),
      click: item => {
        writeShowDockIcon(item.checked)
        applyDockVisibility(item.checked)
      }
    },
    { type: 'separator' },
...
// ─── Notch HUD IPC ─────────────────────────────────────────────────────────
ipcMain.handle('hermes:notch:open', async () => {
  summonJarvis()
  return { ok: true }
})
ipcMain.handle('hermes:notch:close', async () => {
  closeNotch()
  return { ok: true }
})
// Main renderer → notch: push the latest voice/orb state for the HUD to render.
ipcMain.on('hermes:notch:state', (_event, payload) => {
  if (notchWindow && !notchWindow.isDestroyed()) {
    notchWindow.webContents.send('hermes:notch:state', payload)
  }
})
// Click-through: interactive only over the actual HUD content; transparent
// margins pass clicks to whatever is behind.
ipcMain.on('hermes:notch:ignore-mouse', (_event, ignore) => {
  if (notchWindow && !notchWindow.isDestroyed()) {
    notchWindow.setIgnoreMouseEvents(Boolean(ignore), { forward: true })
  }
})
// Flip focusable while the notch mini-composer needs the keyboard, then back.
ipcMain.on('hermes:notch:set-focusable', (_event, focusable) => {
  if (!notchWindow || notchWindow.isDestroyed()) {
    return
  }
  notchWindow.setFocusable(Boolean(focusable))
  if (focusable) {
    notchWindow.focus()
  }
})
// Notch → main renderer: control messages (close, open the app, submit text).
ipcMain.on('hermes:notch:control', (_event, payload) => {
  ...
  if (mainWindow && !mainWindow.isDestroyed()) {
    mainWindow.webContents.send('hermes:notch:control', payload)
  }
})
// ─── Hand-control overlay IPC ──────────────────────────────────────────────
ipcMain.handle('hermes:handtrack:open', async () => {
  openHandtrack()
  return { ok: true }
})
ipcMain.handle('hermes:handtrack:close', async () => {
  closeHandtrack()
  return { ok: true }
})
...
// Camera access for the hand-control feature (mirrors the mic prompt).
ipcMain.handle('hermes:requestCameraAccess', async () => {
  if (!IS_MAC || typeof systemPreferences.askForMediaAccess !== 'function') {
    return true
  }
...
// ─── Installed-app enumeration + launch (for the hand-control app wheel) ────

// Cache the scan briefly — /Applications rarely changes within a session and
// getFileIcon per app is comparatively expensive.
let _installedAppsCache = null
let _installedAppsCacheAt = 0
const INSTALLED_APPS_TTL_MS = 60_000

async function enumerateInstalledApps() {
  const now = Date.now()
  if (_installedAppsCache && now - _installedAppsCacheAt < INSTALLED_APPS_TTL_MS) {
  ...
  for (const dir of dirs) {
    ...
    for (const entry of entries) {
      ...
      seen.add(name)
      apps.push({ name, path: path.join(dir, entry.name) })
    }
  }

  apps.sort((a, b) => a.name.localeCompare(b.name))

  // Attach icons as data URLs (best effort; skip on failure).
  const withIcons = await Promise.all(
    apps.map(async a => {
      let icon = ''
      try {
        const img = await app.getFileIcon(a.path, { size: 'normal' })
        if (img && !img.isEmpty()) {
          icon = img.toDataURL()
        }
      } catch {
      ...
      return { name: a.name, path: a.path, icon }
    })
  )

  _installedAppsCache = withIcons
  _installedAppsCacheAt = now
  return withIcons
}

ipcMain.handle('hermes:apps:list', async () => {
  try {
    return { ok: true, apps: await enumerateInstalledApps() }
  } catch (err) {
    rememberLog(`[apps] list failed: ${err?.message || err}`)
    return { ok: false, apps: [], error: String(err?.message || err) }
  }
})

// The curated app selection for the hand-control wheel (array of app names).
function readSelectedApps() {
  try {
    const raw = fs.readFileSync(DESKTOP_CONTROL_APPS_PATH, 'utf8')
    const parsed = JSON.parse(raw)
    if (Array.isArray(parsed?.apps)) {
      return parsed.apps.filter(n => typeof n === 'string')
    }
  } catch {
    /* no file yet */
  }
...
ipcMain.handle('hermes:apps:selection:get', async () => ({ ok: true, apps: readSelectedApps() }))

ipcMain.handle('hermes:apps:selection:set', async (_event, names) => {
  const apps = Array.isArray(names) ? names.filter(n => typeof n === 'string') : []
  try {
    fs.writeFileSync(DESKTOP_CONTROL_APPS_PATH, JSON.stringify({ apps }), 'utf8')
    return { ok: true }
  } catch (err) {
    rememberLog(`[apps] selection save failed: ${err?.message || err}`)
    return { ok: false, error: String(err?.message || err) }
  }
})

// Launch an installed app by name (or absolute .app path) via `open`.
ipcMain.handle('hermes:apps:launch', async (_event, target) => {
  ...
  try {
    if (IS_MAC) {
      // `open -a <name>` resolves by app name; a path launches that bundle.
      const args = value.endsWith('.app') || value.startsWith('/') ? [value] : ['-a', value]
    ...
    rememberLog(`[apps] launch failed: ${err?.message || err}`)
    return { ok: false, error: String(err?.message || err) }
  }
})
...
ipcMain.handle('hermes:deep-link-ready', () => {
  ...
  // Flush a tray-requested route that arrived before this (recreated) renderer
  // was listening.
  if (_pendingTrayNav && mainWindow && !mainWindow.isDestroyed()) {
    const route = _pendingTrayNav
    _pendingTrayNav = null
    try {
      mainWindow.webContents.send('hermes:tray-navigate', route)
    } catch (err) {
      rememberLog(`[tray] deferred navigate failed: ${err?.message || err}`)
    }
  }
  return { ok: true }
})
...
app.whenReady().then(() => {
  ...
  // Menu-bar icon is always present; the Dock icon is optional (persisted).
  createTray()
  applyDockVisibility(readShowDockIcon())

  // The notch HUD is always present (NotchNook-style): a black pill that hugs the
  // notch and springs open on hover / voice activity. Opened after a short beat
  // so the renderer + display metrics are ready.
  setTimeout(() => {
    if (!app.isReady()) {
      return
    }
  ...
  // Global hotkey to summon Jarvis (notch HUD + voice) from anywhere. ⌥Space is
  // free on macOS (Spotlight is ⌘Space); if it's already taken, log and move on
  // — the menu-bar "Talk to Jarvis" item is always available as a fallback.
  try {
    if (!globalShortcut.register('Alt+Space', () => summonJarvis())) {
      rememberLog('[hotkey] Alt+Space not registered (already in use)')
    }
  } catch (err) {
    rememberLog(`[hotkey] register failed: ${err?.message || err}`)
  }
...
app.on('before-quit', () => {
  ...
  // Release the global hotkey + remove the menu-bar icon so neither lingers.
  try {
