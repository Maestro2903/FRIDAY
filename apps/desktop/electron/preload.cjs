...
contextBridge.exposeInMainWorld('hermesDesktop', {
  ...
  notch: {
    // Summon / dismiss the notch HUD window.
    open: () => ipcRenderer.invoke('hermes:notch:open'),
    close: () => ipcRenderer.invoke('hermes:notch:close'),
    // Main renderer → notch: push the latest voice/orb state.
    pushState: payload => ipcRenderer.send('hermes:notch:state', payload),
    // Notch → main renderer (forwarded by main): open app / submit / close.
    control: payload => ipcRenderer.send('hermes:notch:control', payload),
    // Notch: toggle click-through as the pointer enters/leaves the HUD content.
    setIgnoreMouse: ignore => ipcRenderer.send('hermes:notch:ignore-mouse', ignore),
    // Notch: flip focusable while its mini-composer needs the keyboard.
    setFocusable: focusable => ipcRenderer.send('hermes:notch:set-focusable', focusable),
    // Notch subscribes to state pushes from the main renderer.
    onState: callback => {
      const listener = (_event, payload) => callback(payload)
      ipcRenderer.on('hermes:notch:state', listener)
      return () => ipcRenderer.removeListener('hermes:notch:state', listener)
    },
    // Main renderer subscribes to notch control messages.
    onControl: callback => {
      const listener = (_event, payload) => callback(payload)
      ipcRenderer.on('hermes:notch:control', listener)
      return () => ipcRenderer.removeListener('hermes:notch:control', listener)
    }
  },
  ...
  requestCameraAccess: () => ipcRenderer.invoke('hermes:requestCameraAccess'),
  // Installed-app enumeration + launch + the curated wheel selection.
  apps: {
    list: () => ipcRenderer.invoke('hermes:apps:list'),
    launch: target => ipcRenderer.invoke('hermes:apps:launch', target),
    getSelection: () => ipcRenderer.invoke('hermes:apps:selection:get'),
    setSelection: names => ipcRenderer.invoke('hermes:apps:selection:set', names)
  },
  // The hand-control "control panel" full-screen overlay.
  handtrack: {
    open: () => ipcRenderer.invoke('hermes:handtrack:open'),
    close: () => ipcRenderer.invoke('hermes:handtrack:close')
  },
  ...
  // Menu-bar (tray) menu → navigate the main window to a HashRouter route.
  onTrayNavigate: callback => {
    const listener = (_event, route) => callback(route)
    ipcRenderer.on('hermes:tray-navigate', listener)
    return () => ipcRenderer.removeListener('hermes:tray-navigate', listener)
  },
