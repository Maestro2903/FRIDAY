...
import type { NotchControl, NotchState } from './store/notch'

export {}

declare global {
  // An installed application surfaced for the hand-control app wheel.
  interface InstalledApp {
    name: string
    path: string
    /** data: URL of the app icon, or '' if unavailable. */
    icon: string
  }
  ...
  interface Window {
    hermesDesktop: {
      ...
      // The Siri-style notch HUD: a transparent always-on-top window that mirrors
      // the voice orb's state (pushed from the main renderer) and sends control
      // messages back (open the app, submit typed text, close).
      notch: {
        open: () => Promise<{ ok: boolean }>
        close: () => Promise<{ ok: boolean }>
        pushState: (payload: NotchState) => void
        control: (payload: NotchControl) => void
        setIgnoreMouse: (ignore: boolean) => void
        setFocusable: (focusable: boolean) => void
        onState: (callback: (payload: NotchState) => void) => () => void
        onControl: (callback: (payload: NotchControl) => void) => () => void
      }
      ...
      apps: {
        list: () => Promise<{ ok: boolean; apps: InstalledApp[]; error?: string }>
        launch: (target: string) => Promise<{ ok: boolean; error?: string }>
        getSelection: () => Promise<{ ok: boolean; apps: string[] }>
        setSelection: (names: string[]) => Promise<{ ok: boolean; error?: string }>
      }
      // The hand-control "control panel" full-screen overlay window.
      handtrack: {
        open: () => Promise<{ ok: boolean }>
        close: () => Promise<{ ok: boolean }>
      }
      ...
      onTrayNavigate?: (callback: (route: string) => void) => () => void
