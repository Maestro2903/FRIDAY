...
const VoiceView = lazy(async () => ({ default: (await import('./voice')).VoiceView }))
...
export function DesktopController() {
  ...
  // Menu-bar (tray) menu item → jump to the requested HashRouter route (chat,
  // voice, settings). The main process focuses/creates the window first.
  useEffect(() => {
    const unsubscribe = window.hermesDesktop?.onTrayNavigate?.(route => {
  ...
  const overlays = (
    <>
      ...
      {/* Nous Portal / provider sign-in onboarding removed — the app uses a
          preconfigured provider (see ~/.hermes/config.yaml), so no login is
          prompted. */}
