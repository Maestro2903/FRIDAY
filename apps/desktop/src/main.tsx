...
} else if (winParam === 'notch') {
  // The Siri-style notch HUD rides the same bundle (`?win=notch`) as a minimal,
  // transparent, gateway-less surface that only mirrors voice state.
  void import('./app/notch/notch-root').then(({ mountNotch }) => mountNotch())
} else if (winParam === 'handtrack') {
  // The hand-control "control panel": a full-screen transparent overlay with a
  // webcam hand skeleton + a radial app wheel.
  void import('./app/handtrack/handtrack-root').then(({ mountHandtrack }) => mountHandtrack())
} else {
