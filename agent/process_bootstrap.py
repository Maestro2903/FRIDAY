...
def build_keepalive_http_client(
    ...
    *,
    ...
    try:
        ...
        # OpenCode Zen's free gateway (opencode.ai/zen) 401s on any
        # Authorization header and only works with it absent; the OpenAI SDK
        # always sends one. Strip it for this host so aux calls (title,
        # compression, etc.) work against the keyless free model. Scoped to
        # opencode.ai — all other providers keep their auth header.
        extra_client_kwargs: dict = {}
        if "opencode.ai" in base_lower:
            if async_mode:
                async def _strip_auth_header(request: Any) -> None:
                    request.headers.pop("authorization", None)
            else:
                def _strip_auth_header(request: Any) -> None:  # type: ignore[misc]
                    request.headers.pop("authorization", None)
            extra_client_kwargs["event_hooks"] = {"request": [_strip_auth_header]}

        if "api.githubcopilot.com" in base_lower:
            client_cls = httpx.AsyncClient if async_mode else httpx.Client
            return client_cls(verify=verify, **extra_client_kwargs)
