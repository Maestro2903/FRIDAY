...
TOOL_USE_ENFORCEMENT_GUIDANCE = (
    ...
    "Acting includes asking. When an action needs a specific target you were not "
    "given and cannot look up (which song, which file, which recipient and message, "
    "which of several matches), asking one short clarifying question IS making "
    "progress — not stalling. Do not execute an action on a guessed target; get the "
    "missing detail first, then act."
)
...
def active_voice_persona() -> str:
    """The configured assistant persona: 'jarvis' (default, male) or 'friday'
    (female). Read from ``voice.persona`` in the Hermes config. Drives which SOUL
    file is loaded and the fast voice-reply system prompt."""
    try:
        from hermes_cli.config import load_config
        ...
        if isinstance(voice_cfg, dict):
            persona = str(voice_cfg.get("persona") or "").strip().lower()
        return "friday" if persona == "friday" else "jarvis"
    except Exception:
        return "jarvis"
...
def load_soul_md(context_length: Optional[int] = None) -> Optional[str]:
    ...
    # Persona selection: 'friday' loads SOUL_FRIDAY.md (female persona) when it
    # exists; anything else (default) uses the JARVIS SOUL.md. Voice is chosen
    # separately by the tts.* config.
    soul_path = get_hermes_home() / "SOUL.md"
    if active_voice_persona() == "friday":
        friday_path = get_hermes_home() / "SOUL_FRIDAY.md"
        if friday_path.exists():
            soul_path = friday_path
